...
public class RedisRuleBasedOrderAddProductAction extends GeneratedRedisRuleBasedOrderAddProductAction implements JaloOnlyItem
{
	private final static Logger LOG = Logger.getLogger(RedisRuleBasedOrderAddProductAction.class.getName());
			...
			throws JaloBusinessException
	{
		...
		if (!checkMandatoryAttribute(AbstractPromotionAction.MARKEDAPPLIED, allAttributes, missing)
				...
				| !checkMandatoryAttribute(QUANTITY, allAttributes, missing))
		{
		...
		try
		{
			final RedisRuleBasedOrderAddProductAction newOne = (RedisRuleBasedOrderAddProductAction) cl.newInstance();
	...
	@Override
	public void setProduct(final SessionContext ctx, final Product product)
	{
		data.setProperty(ctx, PRODUCT, product);
	}
	...
	@Override
	public void setQuantity(final SessionContext ctx, final Long product)
	{
		data.setProperty(ctx, QUANTITY, product);
	}
	...
	protected AbstractPromotionAction deepClone(final SessionContext ctx)
	{
		try
		{
			...
			final ComposedType type = TypeManager.getInstance().getComposedType(RedisRuleBasedOrderAddProductAction.class);
			try
			{
